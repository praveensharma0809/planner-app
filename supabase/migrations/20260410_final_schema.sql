...
create table if not exists public.profiles (
  ...
  constraint profiles_daily_available_minutes_check check (daily_available_minutes > 0)
);
...
create unique index if not exists uq_subjects_user_name_active
  on public.subjects (user_id, lower(name))
  where archived = false;

create index if not exists idx_subjects_user_archived_sort
  on public.subjects (user_id, archived, sort_order);

create index if not exists idx_subjects_user_deadline
  on public.subjects (user_id, deadline);

create index if not exists idx_topics_user_subject_sort
  on public.topics (user_id, subject_id, sort_order);

create index if not exists idx_topics_user_archived
  on public.topics (user_id, archived);

create index if not exists idx_topics_user_deadline
  on public.topics (user_id, deadline);

create index if not exists idx_topics_depends_on_gin
  on public.topics using gin (depends_on);

create index if not exists idx_topic_tasks_user_topic_sort
  on public.topic_tasks (user_id, topic_id, sort_order, created_at);

create index if not exists idx_topic_tasks_user_subject
  on public.topic_tasks (user_id, subject_id);

create index if not exists idx_topic_tasks_user_completed
...
create index if not exists idx_tasks_user_source_date
  on public.tasks (user_id, task_source, scheduled_date);

create index if not exists idx_tasks_plan_snapshot_id
  on public.tasks (plan_snapshot_id);

create index if not exists idx_tasks_user_source_topic_task
...
create unique index if not exists uq_plan_snapshots_user_commit_hash
  on public.plan_snapshots (user_id, commit_hash)
  where commit_hash is not null;

create index if not exists idx_plan_snapshots_user_commit_hash_created
  on public.plan_snapshots (user_id, commit_hash, created_at desc)
  where commit_hash is not null;

create index if not exists idx_off_days_user_date
...
create index if not exists idx_ops_events_name_created
...
drop trigger if exists topic_tasks_set_updated_at on public.topic_tasks;
create trigger topic_tasks_set_updated_at
before update on public.topic_tasks
for each row execute function public.set_updated_at();

drop trigger if exists planner_settings_set_updated_at on public.planner_settings;
...
drop policy if exists topics_owner_select on public.topics;
...
drop policy if exists topic_tasks_owner_select on public.topic_tasks;
create policy topic_tasks_owner_select on public.topic_tasks
for select using (auth.uid() = user_id);

drop policy if exists topic_tasks_owner_insert on public.topic_tasks;
create policy topic_tasks_owner_insert on public.topic_tasks
for insert with check (auth.uid() = user_id);

drop policy if exists topic_tasks_owner_update on public.topic_tasks;
create policy topic_tasks_owner_update on public.topic_tasks
for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists topic_tasks_owner_delete on public.topic_tasks;
create policy topic_tasks_owner_delete on public.topic_tasks
for delete using (auth.uid() = user_id);

drop policy if exists planner_settings_owner_select on public.planner_settings;
create policy planner_settings_owner_select on public.planner_settings
for select using (auth.uid() = user_id);

drop policy if exists planner_settings_owner_insert on public.planner_settings;
...
drop policy if exists planner_settings_owner_delete on public.planner_settings;
create policy planner_settings_owner_delete on public.planner_settings
for delete using (auth.uid() = user_id);

drop policy if exists plan_snapshots_owner_select on public.plan_snapshots;
create policy plan_snapshots_owner_select on public.plan_snapshots
for select using (auth.uid() = user_id);

drop policy if exists plan_snapshots_owner_insert on public.plan_snapshots;
create policy plan_snapshots_owner_insert on public.plan_snapshots
for insert with check (auth.uid() = user_id);

drop policy if exists plan_snapshots_owner_update on public.plan_snapshots;
create policy plan_snapshots_owner_update on public.plan_snapshots
for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists plan_snapshots_owner_delete on public.plan_snapshots;
...
drop policy if exists ops_events_owner_select on public.ops_events;
create policy ops_events_owner_select on public.ops_events
for select using (auth.uid() = user_id);

drop policy if exists ops_events_owner_insert on public.ops_events;
create policy ops_events_owner_insert on public.ops_events
for insert with check (user_id is null or auth.uid() = user_id);

drop policy if exists ops_events_owner_update on public.ops_events;
create policy ops_events_owner_update on public.ops_events
for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists ops_events_owner_delete on public.ops_events;
...
drop function if exists public.commit_plan_atomic(uuid, jsonb, text, jsonb, text, date);
drop function if exists public.commit_plan_atomic(jsonb, text, jsonb, text, date);
drop function if exists public.commit_plan_atomic(uuid, jsonb, text, jsonb);
drop function if exists public.commit_plan_atomic_v2_wrapper(jsonb, text, jsonb, text, date);
drop function if exists public.commit_plan_atomic_v2(jsonb, text, jsonb, text, date);
drop function if exists public.commit_plan_atomic_v2(jsonb, text, jsonb, text, date, text);
...
begin
  ...
  if exists (
    ...
    where case
      when trim(coalesce(item->>'session_number', '')) ~ '^[0-9]+$'
        and trim(coalesce(item->>'total_sessions', '')) ~ '^[0-9]+$'
        then (trim(item->>'session_number'))::integer <= 0
  ...
  with parsed_tasks as (
    select
      ...
      (trim(item->>'session_number'))::integer as session_number,
      (trim(item->>'total_sessions'))::integer as total_sessions
    from jsonb_array_elements(coalesce(p_tasks, '[]'::jsonb)) as payload(item)
  ),
  normalized_tasks as (
    select
...
drop function if exists public.sync_topic_task_completion(uuid, boolean);
