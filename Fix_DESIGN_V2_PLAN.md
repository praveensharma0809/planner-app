...
- 2026-05-06 · F8.1 · DSP · PASS — Applied --accent-selected-* tokens (warm sand bg #EBE2D2 + #2A2418 fg) to: sidebar active nav (already done in F2.7), NavigationColumn active rows in both dashboard/subjects and planner (replaced bg-pastel-lilac/60 + text-pastel-lilac-text), Calendar today cell and agenda today badge (replaced --pastel-butter refs), Schedule current day column header (desktop + mobile), Planner current phase button (.planner-topbar-phase-btn-active CSS). Removed all lilac/peach/butter ad-hoc selected colors.
...
- 2026-05-06 · F11.1 · DSP · PASS — Breadcrumb polish: truncated path display at small widths (intermediate items shrink with text-overflow:ellipsis, last item always visible with flex-shrink:0), responsive font sizes (12px→11px<md→10px<400px), extracted BreadcrumbNav component with fixed CSS classes
- 2026-05-06 · F11.2 · DSP · PASS — Action area "+ New Plan" button: full text at >=md (32px pill), "+" icon + "New" label at sm (640-767px), icon-only FAB positioned fixed bottom-right at <640px with 48px touch target and box-shadow
- 2026-05-06 · F11.3 · DSP · PASS — Phase pills: horizontal scroll (overflow-x:auto with hidden scrollbar) above 480px, flex-wrap below 480px; reduced to 26px height / 10px font / 8px padding to fit 56px topbar
- 2026-05-06 · F11.4 · DSP · PASS — Topbar height locked at h-14 (56px) across all routes and widths: --topbar-height changed 64px→56px, schedule topbar converted from 92px grid layout to flex column with min-height 56px, all schedule controls reduced (24px pills, 10px font, tighter padding), phase/reset buttons reduced to 26px/36px mobile
- 2026-05-06 · Gate F11 · DSP · PASS — All 200/200 tests pass. TypeScript typecheck clean. Topbar consistent 56px across 6 routes. Breadcrumbs truncate gracefully. "+ New" button accessible at all widths via responsive states.
---
...
- 2026-05-06 · R1 · DSP · PASS — Calendar grid event caps (§F9.1): at lg (1024–1279) day cells show max 2 events + "+N more" chip; at xl+ (≥1280) show max 4 events; below lg agenda view handles it. MonthView.tsx updated with responsive event slots and overflow counts per breakpoint.
- 2026-05-06 · R2 · DSP · PASS — Schedule day-column count (§F9.2): at lg (1024) show 5 day columns (Mon–Fri); at xl+ (≥1280) show 7 day columns (Mon–Sun). Below lg single-column vertical agenda (F7.6) unchanged. Used hidden lg:grid xl:hidden / hidden xl:grid patterns for both header and body grid.
- 2026-05-06 · R3 · DSP · PASS — WCAG AA contrast re-audit (§F10.1): Full audit against warm canvas #F4F1EA and white cards #FFFFFF. Fixed text-muted #8D95A5→#5F6B7A (4.76:1 on canvas, 5.37:1 on card). Fixed sidebar inactive nav text rgba(0,0,0,0.45)→#596577 (5.18:1 on canvas). Added sidebar-nav-active and sidebar-nav-inact rows. All 16 pairs ≥4.5:1. Audit table updated in globals.css header comment.
- 2026-05-06 · R4 · DSP · PASS — Focus rings (§F10.2): Changed --focus-ring from #111827 to #3B5CFF in both :root and @theme inline blocks. Existing *:focus-visible rules with outline 2px solid var(--focus-ring) and outline-offset: 2px verified across 20+ selectors (sidebar nav items, tab bar, buttons, inputs, select dropdowns, calendar cells, schedule day columns, data-table rows, drag handles, chevron toggles, sort toggles).
- 2026-05-06 · R5 · DSP · PASS — Final screenshot capture (§F11.1): Created scripts/capture-post-fix-v2.mjs for 6 routes × 5 widths capture into app_screenshots/Post_Fix_V2/. Created PRE_POST_COMPARISON.md with side-by-side comparison per-route/per-width documenting changes, improvements, and remaining issues against Pre_Fix_V2/ baseline.
- 2026-05-06 · R6 · DSP · PASS — Final plan review (§F11.2): Verified every task DoD in Fix_DESIGN_V2_PLAN.md §3 and §4 (all 62 tasks complete). Validation gates G1–G10 all recorded as PASS. Residual tasks R1–R6 complete the repurposed F9.1/F9.2/F10.1/F10.2/F11.1/F11.2. All 200/200 tests pass. Branch ready for merge.
